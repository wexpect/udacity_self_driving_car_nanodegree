...
int main(){
	...
	lidar->Listen([&new_scan, &lastScanTime, &scanCloud](auto data){
		...
		if(new_scan){
			...
			std::cout << "pclCloud has " << pclCloud.size() << " points" << std::endl;
	...
	while (!viewer->wasStopped())
  	{
		std::cout << "before while, new_scan = " << new_scan << std::endl;
		while(new_scan){
			std::this_thread::sleep_for(0.1s);
			world.Tick(1s);
		}
		std::cout << "afte while, new_scan = " << new_scan << std::endl;
		...
		drawCar(truePose, 0,  Color(0,1,0), 0.7, viewer);
		...
		if(!new_scan){
			...
			cout << "scanCloud has " << scanCloud->points.size() << " points" << endl;
			...
			// NOTE: ICP
			Eigen::Matrix4d transformation_matrix = ICP(mapCloud, cloudFiltered, pose, 10); 
			// NOTE: TODO add NDP, make be better
			...
			// NOTE: not accumulate scan points
