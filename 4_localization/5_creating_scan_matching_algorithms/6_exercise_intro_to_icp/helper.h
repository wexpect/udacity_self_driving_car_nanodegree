...
struct Pose{
	...
	void Print(){
		cout << "x: " << position.x << " y: " << position.y << " theta: " << theta << endl;
	}
...
// Note: lidar here is scanning 2D point clouds
struct Lidar{
	...
	// NOTE: scan is in lidar coordinate system, x is forward, y is to left
	...
	void Move(double step, double rotate){
		...
		c$out << "before, x: " << x << " y: " << y << " theta: " << theta << endl;				
		cout << "step: " << step << " rotate: " << rotate << endl;		

		theta += rotate;
		x += step * cos(theta);
		y += step * sin(theta);

    	cout << "delta_x: " << step * cos(theta) << " delta_y: " << step * sin(theta) << " delta_theta: " << rotate << endl;		
		cout << "after, x: " << x << " y: " << y << " theta: " << theta << endl;		
	}
};