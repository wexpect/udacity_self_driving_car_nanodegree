...
Eigen::Matrix4d ICP(PointCloudT::Ptr target, PointCloudT::Ptr source, Pose startingPose, int iterations){
	...
	if( icp.hasConverged() ){
		...
		std::cout << "ICP not converge" << std::endl;
	}
...
int main(){
	...
	drawCar(truePose[0], 0, Color(0,1,0), 0.7, viewer);   
	std::cout << "truePose[0] " << truePose[0].position.x  << ", "<< truePose[0].position.y  << ", "<< truePose[0].position.z  << ", "<< truePose[0].rotation.yaw << ", "<< truePose[0].rotation.pitch << ", "<< truePose[0].rotation.roll << std::endl;

	// Load input scan
	PointCloudT::Ptr scanCloud(new PointCloudT);
  	pcl::io::loadPCDFile("scan1.pcd", *scanCloud);
  	cout << "Loaded " << scanCloud->points.size() << " data points from scan1.pcd" << endl;
	...
	vg.filter(*cloudFiltered);
  	...
  	cout << "cloudFiltered has " << cloudFiltered->points.size() << " data points" << endl;
	...
	std::cout << "start, pose estimation " << pose.position.x  << ", "<< pose.position.y  << ", "<< pose.position.z  << ", "<< pose.rotation.yaw << ", "<< pose.rotation.pitch << ", "<< pose.rotation.roll << std::endl;
	
	while (!viewer->wasStopped()){
		// NOTE: the loop keeps running
		...
		if( matching != Off){
			std::cout <<"Checkpoint 1, matching " << matching << std::endl;
			...
			std::cout << "new pose estimation " << pose.position.x  << ", "<< pose.position.y  << ", "<< pose.position.z  << ", "<< pose.rotation.yaw << ", "<< pose.rotation.pitch << ", "<< pose.rotation.roll << std::endl;

			if( !tester.Displacement(pose) ){
				if(matching == Icp)
					cout << "Whole process is done" << endl;
				...
				std::cout <<"displacement true" << std::endl;
			}
			std::cout <<"Checkpoint 2, matching " << matching << std::endl;
		}		
		...
		drawCar(pose, 1,  Color(1,0,0), 0.35, viewer);  
