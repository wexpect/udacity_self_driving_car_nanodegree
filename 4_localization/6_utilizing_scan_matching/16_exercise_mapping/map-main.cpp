...
int main(){
		...
		if(new_scan){
			...
			for (auto detection : *scan){
				...
				if((detection.point.x*detection.point.x + detection.point.y*detection.point.y + detection.point.z*detection.point.z) > 8.0){ // Don't include points touching ego
					...
					// NOTE: keep adding points
					pclCloud.points.push_back(PointT(transform_point[0], transform_point[1], transform_point[2]));

					pointCount += 1;
				}
			}
			std::cout << "one scan has " << pointCount << " points" << std::endl;						
			std::cout << "pclCloud has accumulated " << pclCloud.points.size() << " points" << std::endl;
	...
	*scanCloud = pclCloud;
  	...
  	cout << "scanCloud has " << scanCloud->points.size() << " data points" << endl;
