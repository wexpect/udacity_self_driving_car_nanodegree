...
double calc_theta(const double s, const std::array<double, 4>& a) {
  ...
  double theta_s = a[3] * pow(s, 4) / 4 + a[2] * pow(s, 3) / 3 + a[1] * pow(s, 2) / 2 + a[0] * s;
...
std::vector<double> generate_f_s0_sn(const double delta_s,
  ...
  for (size_t i = 0; i <= n; ++i) {
    ...
    // f(x) = dx / 3.0 * (f(x0) + 4.0 * f(x1) + 2.0 * f(x2) + 4.0 * f(x3) + 2.0 * f(x4) + ... + func[n]);
    // dx = sf/n;
    // xi = i*dx;
    // HINT: s_i = i * delta_s;
...
int main(int argc, const char* argv[]) {
  ...
  std::cout << "integral_result: " << integral_result << std::endl;
