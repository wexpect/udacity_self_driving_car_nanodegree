...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  auto look_ahead_distance = ( pow(0, 2) - pow(velocity_mag, 2) ) / ( 2 * -1 * accel_mag);  // <- Fix This  
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    // NOTE: seems BP means behavior planner
    ...
    if (is_goal_in_junction) {
      ...
      // NOTE: cos and sin are negative in Quadrant 3
      goal.location.x = goal.location.x + _stop_line_buffer * cos(ang);  // <- Fix This
      goal.location.y = goal.location.y + _stop_line_buffer * sin(ang);  // <- Fix This      
    ...
    // speed. Make sure the distance to the stopping point is <= P_STOP_THRESHOLD_DISTANCE. 
    
    // TODO-calculate the distance. Uncomment the line used to calculate the distance
