...
def PTG(start_s, start_d, target_vehicle, delta, T, predictions):
    ...
    return:
     ...
     best_d gives coefficients for d(t)
     and best_t gives duration associated w/ this trajectory.
    """
    ...
    num_step = 4  # 4  # NOTE: from T - 2 sec to T + 2 sec
    t = T - num_step * timestep
    while t <= T + num_step * timestep:
        # NOTE: get ego target_state
    ...
    # NOTE: actually means trajectory coefficients
    trajectories = []
    ...
    for traj in trajectories:
        ...
        cost = calculate_cost(traj, target_vehicle, delta, T, predictions, WEIGHTED_COST_FUNCTIONS, verbose=True)
        print("weighted cost ", cost)

        show_trajectory(traj[0], traj[1], traj[2], vehicle=None, customer_color="blue")
    ...
    best_cost = calculate_cost(best, target_vehicle, delta, T, predictions, WEIGHTED_COST_FUNCTIONS, verbose=True)
