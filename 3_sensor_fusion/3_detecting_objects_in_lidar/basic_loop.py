...
data_filenames = [
    ...
    'training_segment-10017090168044687777_6380_000_6400_000_with_camera_labels.tfrecord', # Sequence 4
]
...
#         l1_exercises.vis_intensity_channel(frame, lidar_name)
        ...
        # -0.3 will remove too many points
#         configs.lim_z = [-0.3, 3]
        # later in course, seems Udacity code uses [-1, 3]
#         configs.lim_z = [-1, 3] 
        configs.lim_z = [-3, 3] 
        ...
        """ a large number of pre-computed files which contain TP, FP and FN for ~200 frames of sequence 1 for several settings of the confidence threshold level used in object detection to decide when to keep object candidates
        
        a variable containing the current confidence threshold (used for object detection) is appended. This makes it possible to load the TP, FP and FN for various confidence levels such that we can arrive at an estimate of the average precision
        """
