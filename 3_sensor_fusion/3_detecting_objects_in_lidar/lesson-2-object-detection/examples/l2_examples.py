...
def render_obj_over_bev(detections, lidar_bev_labels, configs, vis=False):
    ...
    # NOTE: lidar_bev_labels is the returned bev_map_cpy above, meaning BEV map with labels drawn
    # NOTE: color channel is in BGR
...
#         cv2.imshow("BEV map", lidar_bev_labels)
        cv2.imshow("BEV map", cv2.rotate(lidar_bev_labels, cv2.ROTATE_180))
...
def render_bb_over_bev(bev_map, labels, configs, vis=False):
    ...
    # NOTE: here only use vehicle labels
    # return value is [label.type, label.box.center_x, label.box.center_y, label.box.center_z,
    # label.box.height, label.box.width, label.box.length, label.box.heading]
    # return values are in meters
...
#         cv2.imshow("BEV map", bev_map_cpy)
        cv2.imshow("BEV map", cv2.rotate(bev_map_cpy, cv2.ROTATE_180))
...
def crop_pcl(lidar_pcl, configs, vis=True):
    ...
    print('before crop, lidar_pcl.shape', lidar_pcl.shape)
    ...
    print('after crop, lidar_pcl.shape', lidar_pcl.shape)

    # visualize point-cloud
    if vis:
        pcd = o3d.geometry.PointCloud()

        # if use liar_pcl, will throw error
