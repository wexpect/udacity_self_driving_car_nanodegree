...
def project_detections_into_bev(bev_map, detections, configs, color=[]):
    for row in detections:
        # extract detection
        # NOTE:
        # row is [type, center_x, enter_y, center_z, height, width, length, heading]
        # row values are in meters
        _id, _x, _y, _z, _h, _w, _l, _yaw = row


        # convert from metric into pixel coordinates

        # NOTE: from meters to pixel values
        # in the Lidar meter and BEV pixel coordinates, x and y are opposite
...
def is_label_inside_detection_area(label, configs, min_overlap=0.5):
    ...
    # convert detection area into polygon
    ...
    # NOTE: for each label, at least 50% of the area must be located within the BEV map - otherwise it is discarded
