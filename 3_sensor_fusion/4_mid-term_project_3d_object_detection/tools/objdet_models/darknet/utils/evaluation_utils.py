...
def post_processing_v2(prediction, conf_thresh=0.95, nms_thresh=0.4):
    """
        ...
        Non-Maximum Suppression to further filter detections.
        Returns detections with shape:
            (x, y, w, l, im, re, object_conf, class_score, class_pred (i.e. class idx))
    """
    ...
    for image_i, image_pred in enumerate(prediction):
        ...
        print('post_processing_v2', 'image_pred\n', image_pred)

        print('post_processing_v2', 'image_pred[:, 7:].max(dim=1)\n', image_pred[:, 7:].max(dim=1))

        # NOTE: overall score
        # object confidence * max class probability
        ...
        # class_preds is the index, i.e., 0, 1, or 2
